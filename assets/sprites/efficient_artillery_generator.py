...
# Optimized artillery model creation
def create_simple_artillery(name="Artillery", color=(0.7, 0.5, 0.2, 1.0)):
    """Create a simplified artillery model that's efficient to render"""
    # Create artillery base
    bpy.ops.mesh.primitive_cube_add(size=1)
    base = bpy.context.active_object
    base.name = f"{name}_Base"
    base.scale = (1.6, 1.6, 0.4)
    base.location = (0, 0, 0.2)
    
    # Create main body
    bpy.ops.mesh.primitive_cylinder_add(
        radius=0.7,
        depth=0.8,
        vertices=10  # Reduced vertices for better performance
    )
    body = bpy.context.active_object
    body.name = f"{name}_Body"
    body.location = (0, 0, 0.7)
    ...
    barrel.location = (0, 0, 1.0)
    barrel.rotation_euler = (math.radians(45), 0, 0)
    
    # Create supports (simplified)
    for side in [-1, 1]:
        bpy.ops.mesh.primitive_cube_add(size=0.5)
        support = bpy.context.active_object
        support.name = f"{name}_Support_{side}"
        support.scale = (0.2, 1.0, 0.2)
        support.location = (side * 0.8, 0, 0.3)
    
    # Create a material for the artillery
...
def main():
    ...
    # Create artillery with a brown/brass color
    artillery = create_simple_artillery(name="SimpleArtillery", color=(0.7, 0.5, 0.2, 1.0))
    
    # Render from 8 angles
    render_8_angles(artillery, output_dir)
